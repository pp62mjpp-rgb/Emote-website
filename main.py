...
app = Flask(__name__)
app.secret_key = "SUPER_RANDOM_SECRET_CHANGE_ME"
...
# --------------------------------------------------------------
# 3. ULTRA FAST EMOTE SEQUENCE
# --------------------------------------------------------------
async def perform_emote_sequence(uids_list, emote_id, team_code_str):
    global online_writer, whisper_writer, key, iv, region
    
    try:
        # Team join - ultra fast
        EM = await GenJoinSquadsPacket(team_code_str, key, iv)
        await SEndPacKeT(whisper_writer, online_writer, 'OnLine', EM)
        await asyncio.sleep(0.2)  # Ultra short wait for team join
        
        # Create all emote packets first for maximum speed
        emote_packets = []
        for uid_target in uids_list:
            packet = await Emote_k(uid_target, emote_id, key, iv, region)
            emote_packets.append(packet)
        
        # Send all emotes with ultra minimal delay
        for i, packet in enumerate(emote_packets):
            await SEndPacKeT(whisper_writer, online_writer, 'OnLine', packet)
            if i < len(emote_packets) - 1:  # No delay after last emote
        ...
        print(f"Ultra fast emote error: {e}")
        # Emergency leave if error occurs
        try:
            leave = await ExiT(None, key, iv)
            await SEndPacKeT(whisper_writer, online_writer, 'OnLine', leave)
        except:
...
# --------------------------------------------------------------
# 4. LOGIN-REQUIRED DECORATOR (REMOVED - DIRECT ACCESS)
# --------------------------------------------------------------
# Login system removed for direct access

# --------------------------------------------------------------
# 5. ROUTES – DIRECT ACCESS (LOGIN REMOVED)
# --------------------------------------------------------------
@app.route('/')
def root():
...
# --------------------------------------------------------------
# 6. MAIN INDEX PAGE (NO LOGIN REQUIRED)
# --------------------------------------------------------------
@app.route('/index')
def index_page():
    try:
        # Get absolute path to emotes.json
        base_dir = os.path.dirname(os.path.abspath(__file__))
        emotes_path = os.path.join(base_dir, 'emotes.json')
...
# --------------------------------------------------------------
# 7. API ENDPOINTS (NO LOGIN REQUIRED) - UPDATED WITH FAST TIMEOUT
# --------------------------------------------------------------
...
def send_emote():
    ...
    try:
        ...
        future.result(timeout=8)  # Reduced timeout for ultra fast version
        return jsonify({'status':'success','message':f'Ultra fast emote sent for {", ".join(uids_str)}'})
...
def join_and_emote_api():
    ...
    try:
        ...
        future.result(timeout=8)  # Reduced timeout for ultra fast version
        return jsonify({"status":"success","message":"Ultra fast emote command sent"})
    except asyncio.TimeoutError:
        return jsonify({"status":"error","message":"Timeout"}), 500
    except Exception as e:
        return jsonify({"status":"error","message":str(e)}), 500
...
async def MaiiiinE():
    ...
    Uid, Pw = '4258501816', '9CB64723B9766F5AFB55738EC4230FE1AA311C87C77BDDF24A58E4C6A8CF32BD'
    ...
    # Call equie_emote with error handling
    try:
        equie_emote(ToKen, UrL)
    except Exception as e:
        print(f"Note: equie_emote skipped: {e}")
    ...
    os.system('clear')
    print(render('ULTRA FAST BOT', colors=['white', 'red'], align='center'))
    print(f"\n⚡ Ultra Fast Bot Online – UID: {TarGeT} | Name: {acc_name}")
    print("🌐 Web UI: http://localhost:30151")
    print("🚀 Emote Speed: ~1 second for 4 UIDs")
